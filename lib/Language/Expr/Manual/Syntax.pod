...
=item * (Implementation) Currently ternary "?:" operator is not parsed.

See L<Language::Expr::Parser> for more information. As an alternative, you can
use the if() function (availability depends on interpreter/compiler).
